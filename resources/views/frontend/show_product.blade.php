...
@section('styles')
    <style type="text/css">
        ...
        .img_active {
            border: 1px solid rgb(167, 237, 4);
        }
    </style>
@endsection
                        ...
                        <div class="container prod_main">
                            ...
                            <div class="row">
                                    ...
                                    <div class="summary entry-summary">
                                        <div class="list_image_container pd-left">
                                            <div id="main_img_0" class="new_bg_image_overlay main-pic" style="display: block;">
                                                <img src="{{ get_thumb_url_500('products', $product->thumbnail_img, 1) }}"
                                                    class="prdouct_lbox" title="{{ $product->name }}">
                                            </div>
                
                                            @foreach ($product_images as $key => $value)
                                                <div id="main_img_{{ $value->id }}" class="new_bg_image_overlay main-pic"
                                                    style="display: none;">
                                                    <img src="{{ get_thumb_url_500('products', $value->img_name, 1) }}" class="prdouct_lbox"
                                                        @if ($product->imagetitle != '') alt="{{ $product->imagetitle }}" @else alt="{{ $product->name }}" @endif>
                                                </div>
                                            @endforeach
                                        ...
                                        <li class="img_active" onclick="change_img_thumb(0)" id="prod_img_0">
                                            <span>
                                                <div class="product_secondary_images_container">
                                                    <img class="product_secondary_image_sm" alt="{{ $product->name }}"
                                                        src="{{ get_thumb_url_100('products', $product->thumbnail_img, 1) }}">
                                                </div>
                                            </span>
                                        </li>
                
                                        @foreach ($product_images as $key => $value)
                                            <li class="" onclick="change_img_thumb({{ $value->id }})"
                                                id="prod_img_{{ $value->id }}">
                                                <span>
                                                    <div class="product_secondary_images_container">
                                                        <img class="product_secondary_image_sm" alt="{{ $product->name }}"
                                                            src="{{ get_thumb_url_100('products', $value->img_name, 1) }}">
                                                    </div>
                                                </span>
                                            </li>
                                        @endforeach
                                ...
                                <div class="col-md-6 col-sm-12 col-xs-12 my-quote">
                                    ...
                                    <h6>Price {{$product_detail->price}}</h6>
                                    
                                    <div class="p_detail_container">
                                        <p>Beds : {{$product_detail->bed}}</p>
                                        <p>Bath : {{$product_detail->bath}}</p>
                                        <p>Size (Sqft) : {{$product_detail->bath}}</p>
                                        <p>Location : {{$product_detail->address}}</p>
                                    </div>
                                    ...
                                    <button class="btn btn-primary inquiry-pops" data-bs-toggle="modal"
                                        data-bs-target="#contactSupplierModal" href="javascript:">Inquire Now</button>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            @foreach ($product_images as $key => $value)
                $('#main_img_{{ $value->id }} > img').ezPlus({
                    url: '/uploads/images/products/{{ $value->img_name }}',
                    touch: false
                });
            @endforeach
        });
